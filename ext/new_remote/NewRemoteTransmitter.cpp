...
NewRemoteTransmitter::NewRemoteTransmitter(unsigned long address, uint8_t pin, unsigned int periodusec, uint8_t repeats) {
	_address = address;
	_pin = pin;
...
void NewRemoteTransmitter::sendUnit(uint8_t unit, bool switchOn) {
	for (int8_t i = _repeats; i >= 0; i--) {
		_sendStartPulse();
...
void NewRemoteTransmitter::sendDim(uint8_t unit, uint8_t dimLevel) {
	for (int8_t i = _repeats; i >= 0; i--) {
		_sendStartPulse();
...
void NewRemoteTransmitter::sendGroupDim(uint8_t dimLevel) {
	for (int8_t i = _repeats; i >= 0; i--) {
		_sendStartPulse();
...
void NewRemoteTransmitter::_sendUnit(uint8_t unit) {
