...
class NewRemoteTransmitter {
	public:
		...
		*/
		NewRemoteTransmitter(unsigned long address, uint8_t pin, unsigned int periodusec = 260, uint8_t repeats = 4);
		 ...
		 * @param switchOn  True to send "on" signal, false to send "off" signal.
		 */
		void sendUnit(uint8_t unit, bool switchOn);
		 ...
		 */
		void sendDim(uint8_t unit, uint8_t dimLevel);
		 ...
		 * @param dimLevel  [0..15] Dim level. 0 for off, 15 for brightest level.
		 */
		void sendGroupDim(uint8_t dimLevel);

	protected:
		unsigned long _address;		// Address of this transmitter.
		uint8_t _pin;			// Transmitter output pin
		unsigned int _periodusec;	// Oscillator period in microseconds
		uint8_t _repeats;		// Number over repetitions of one telegram
